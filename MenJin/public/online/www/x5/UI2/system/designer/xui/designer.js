						...
						if(result){ 
							...
							if (comClass) {
								...
								if (compPath) {
									...
									__loadDepends([ compPath ],function(){
...
//				console.log("===>"+event.ctrlKey);
		...
		paintComponent : function(config) {
			...
			if (config.requireList && config.requireList.length > 0) {
				...
				__loadDepends(config.requireList,function(){
			...
			var _require = element.getAttribute("_require");
			if(_require){
				this.loadResource(_require);
			}
		...
		loadResource : function(cssItems){
			var values = cssItems.split(",");
			for(var i = 0;i<values.length;i+=1){
				var item = values[i];
				var idx = item.lastIndexOf(".");
				if(idx >0){
					var fileExtend = item.substring(idx+1);
					var filePath = item.substring(0,idx);
					if(fileExtend == 'css' || fileExtend == 'CSS'){
						require(["css!"+item.substring(0,idx)],function(a){
							a.load();
						});			
					}else if(fileExtend == 'js' || fileExtend == 'JS'){
						require([filePath]).load();
					}
				}
			}			
		},
