	...
	Model.prototype._refreshLogFrame = function(result) {
		if ((this.getElementByXid("logDiv").style.display == "block") && (this.building || result)) {
			if (this.building || result.flag) {
				...
				var remoteBuilderServer = config.realAppBuilderServer === config.appBuilderServer;
				this.logFrame.src = require.toUrl("$UI/system/deploy/common/buildLog.j?appName=" + config.appName) + "&serverLog=" + this.showServerLog + "&appBuilderServer="
						+ config.realAppBuilderServer + "&userID=" + config.userID + "&remoteBuildServer=" + remoteBuilderServer + "&r=" + Math.random();
			} else {
	...
	Model.prototype._showBuildStatus = function(result) {
		if (result.flag) {
			if (result.code == "ERROR") {
				this.getElementByXid("info").innerText = "执行失败！，请下方文本框了解具体错误（复制错误信息：Ctrl+A全选Ctrl+C复制）";
				this.getElementByXid("logDiv").style.display = "block";
			} else {
				this.getElementByXid("info").innerText = this.getSucessInfo();
			}
		} else {
			this.getElementByXid("info").innerText = "调用服务请求失败！，请下方文本框了解具体错误（复制错误信息：Ctrl+A全选Ctrl+C复制）";
			this.getElementByXid("logDiv").style.display = "block";
		}
