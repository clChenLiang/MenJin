define(function(require) {
	...
	Model.prototype.finish = function(wizard) {
		...
		if (targetFileName.indexOf('.m', targetFileName.length - 2) === -1) {
			...
			processData.each(function(param) {
				var creFile = param.row.val('crefile') + '.m.w'
				if (index > 1) {
					self.templateEngine.addContext(self.templateFile, "parent", parentFileName+'.m.w', creFile);
				} else {
					self.templateEngine.addContext(self.templateFile, "keys", keys, creFile);
					self.templateEngine.addContext(self.templateFile, "parent", targetFileName + ".w", creFile);
				}
