...
<div xmlns="http://www.w3.org/1999/xhtml" component="$UI/system/components/justep/window/window" design="device:mobile"
  ...
  <div component="$UI/system/components/justep/model/model" xid="model" style="height:auto;left:16px;top:246px;">
    <div component="$UI/system/components/justep/data/data" xid="{{data_xid}}"
      directDelete="true" autoLoad="true" idColumn="fID" onCustomRefresh="mainDataCustomRefresh" onCustomSave="mainDataCustomSave" confirmRefresh="false" limit="10">
      <column label="{{data_xid}}" name="fID" type="String" xid="{{data_xid}}"/>
      <column label="选择" name="calcCheckBox" type="Boolean" xid="calcCheckBox"/>{{#each columns}}
      <column label="{{column_label}}" name="{{column_name}}" type="{{column_type}}"
        xid="{{column_xid}}"/>{{/each}}
      <rule xid="rule1"> 
        <col name="fID" xid="ruleCol1"> 
          <calculate xid="calculate1"> 
            <expr xid="default2">justep.UUID.createUUID()</expr>
          </calculate> 
        </col> 
      </rule> 
    </div>  
