...
define(function(require) {
	...
	WindowOpener.prototype.sendToWindow = function() {
		...
		if (b && this.window) {
			...
			var watchID = setInterval(function(){
				if(typeof self.window.postMessage === 'object' || typeof self.window.postMessage === 'function'){
					self.window.postMessage(JSON.stringify({type: 'WindowOpener', data: self.sendData}), '*');
	...
	WindowOpener.prototype.open = function(options) {
		...
		//打开窗口
		this.window = window.open(this._getUrl(), this.windowId, this._getPrames());
