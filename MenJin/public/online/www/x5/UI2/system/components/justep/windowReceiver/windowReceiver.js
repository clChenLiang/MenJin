...
define(function(require) {
	...
	var WindowReceiver = justep.BindComponent.extend({
		...
		dispose : function() {
			this.getModel().off(Model.MESSAGE_EVENT, this._doReceive, this);
			this.callParent();
		},
		...
		doInit : function(value, bindingContext) {
			this.getModel().on(Model.MESSAGE_EVENT, this._doReceive, this);
		...
		_doReceive : function(event){
			...
			if(event.message){
				...
				//只有WindowDialog时接收参数
				if($.inArray(type, ['WindowDialog']) > -1){
					this._receiveData = event.message.data; 
					this.target = event.message.source;
				}
			}
