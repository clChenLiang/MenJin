			...
			.extend({
				...
				_createReadonly : function() {
					...
					this.readonly = Bind.computed(function() {
						...
						return (modelContext && 'function'===typeof(modelContext.isReadonlyMode) && modelContext.isReadonlyMode()) || (this.definition.readonly instanceof Expr ? Expr.eval(this.definition.readonly, this, ctx) : this.definition.readonly);
					}, this);
				},
				...
				_filterBy : function() {
					...
					if (this.master){
						ret = Bind.computed(function() {
							...
							if (undefined === mr || !isModelConctructed)
								return [];
				...
				saveData : function(options) {
					...
					try {
						...
						if (this.hasListener(Data.EVENT_SAVEDATA)) {
							...
							var ret = this.doSaveData(callback, {
								'onSuccess' : onSuccess,
								'onError' : onError
							});
				...
				_row2Json : function(row,excludeCalculateCol,excludeCols) {
					var ret = {};
					for ( var k in this.defCols) {
						var isExcludeCol = $.isArray(excludeCols)?($.inArray(k,excludeCols)>-1):false;
						if (k !== this.idColumn && (!isExcludeCol && (!excludeCalculateCol||(excludeCalculateCol && !this.isUICalculateCol(k)))))
							ret[k] = row.row[k];
					}
				...
				_rows2Json : function(rows,excludeCalculateCol,excludeCols) {
				...
				toJson : function(option) {
					var changed=false,excludeCalculateCol=false,excludeCols=[];
					...
					this.eachAll(function(data) {
						...
						if (!changed || Data.STATE.EDIT == recordState || Data.STATE.NEW == recordState || Data.STATE.DELETE == recordState) {
							rows.push(this._row2Json(row,excludeCalculateCol,excludeCols));
						}
					}, this);
					// 删除的数据
					...
					if (delDatas.length > 0) {
						...
						for ( var i = 0; i < len; i++) {
							rows.push(this._row2Json(delDatas[i],excludeCalculateCol,excludeCols));
						}
					}
					return this._rows2Json(rows,excludeCalculateCol,excludeCols);
				}
			});
