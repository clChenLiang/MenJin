...
//        this.getElementByXid('button18').style.display="block";   
          var success = function(resultData) {
//          quanData=resultData;  
          console.log(resultData);
          if(flag1){
          for(var i=0;i<resultData.storeName.length;i++){
                var tabObj=document.getElementById("myTab1");//获取添加数据的表格
                var rowsNum = tabObj.rows.length;  //获取当前行数       
                var myNewRow = tabObj.insertRow(rowsNum);//插入新行
                var newTdObj1=myNewRow.insertCell(0);
                newTdObj1.innerHTML="<input type='checkbox' style='width:60px;height:20px;margin-left:35px;' name='chkArr' id='chkArr'"+rowsNum+" style='width:90px' /></br></br>";
                var newTdObj2=myNewRow.insertCell(1);
                newTdObj2.innerHTML="<input type='text' name='nodecode'  id='nodename'"+rowsNum+" style='width:125px;border-style:none;text-align:center;color:#1592FF;font-size:x-large;' value='"+resultData.storeName[i]+"'/></br></br>";
                var newTdObj3=myNewRow.insertCell(2);
                newTdObj3.innerHTML="<input type='text' name='nodename'  id='nodecode'"+rowsNum+" style='width:80px;border-style:none;text-align:center;color:#1592FF;font-size:x-large;' value='"+resultData.peopleName[i]+"'/></br></br>";
//                var newTdObj4=myNewRow.insertCell(3);
//                newTdObj4.innerHTML="<input type='text' name='nodename'  id='nodecode'"+rowsNum+" style='width:80px;border-style:none;text-align:center;color:#1592FF;font-size:x-large;' value='"+resultData.numName[i]+"'/></br></br>";
            }
            flag1=false;
          }
	};
...
//            if (confirm("你确定删除吗？")) { 
	...
	Model.prototype.button13Click = function(event){
      var storeName=this.getElementByXid("input4").value;
	  var peopleName=this.getElementByXid("input3").value;
	  if(storeName===""){
		  alert("请先输入店名");
		  return false;
		  }
	...
	Model.prototype.button4Click = function(event){
	 this.getElementByXid('button8').click();
		var arr1=new Array();
        var arr2=new Array();
        function removeRow(){
            var chkObj=document.getElementsByName("chkArr");
            var tabObj=document.getElementById("myTab1");
            for(var k=0,l=0;k<chkObj.length;k++,l++){
                if(chkObj[k].checked){
                    tabObj.deleteRow(k+1);
                    k=k-1;
